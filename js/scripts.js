...
$.ajax({
  ...
  success: function(data) {
    ...
    setModalData(users).insertAfter('#gallery').hide(); //hide modal initially
...
/**
 * Create search bar incl functioanlity
 */
...
function updateData (nr){
  ...
  $('.modal-text').eq(3).html(`${item.location.street}, ${item.location.city}, ${item.location.state} ${item.location.postcode}`);
...
function toggle(){
  $('.modal-btn-container').on('click', '.btn', function (){
    ...
    } else if ($(this).attr('id') === "modal-prev"){ // If button prev then
      if(pos > 0){
        pos -= 1;
        cardNr = cardsVis[pos];
      } else {
